...
subjectname=[ "math", "science", "health","principle"]
...
def combat(subject,answer,classname,ehealth=3, damage=1):
  ...
  if ap == 2:
    ehealth=5
    m="AP"
    if subject != principle:
      print("This is",m,subjectname)
      next()
  while True:
    print("\033c")
    damage=1
...
def cellphone():
  ...
  if itemlist["notebook"] > 0: 
    ...
    next()
def give_item():
  item_chance=random.choice(items)
  if item_chance=="notebook":
    itemlist["notebook"]+=1
    text("you got a notebook, this heals one heart")
    next()
  elif item_chance=="whiteout":
    itemlist["whiteout"]+=1
    text("you got white out. you can use this to skip a question")
    next()
  elif item_chance=="cellphone":
...
print("")
while True:#the main part of the game
  print("\033c")
